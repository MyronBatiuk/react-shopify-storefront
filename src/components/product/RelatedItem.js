    ...
    return (
      <div className="grid__item medium-up--one-quarter">
        <div className="related-item">
          <Link to={`/products/${item.slug}`}><img className="related-item__image" src={item.featured_image} alt=""/></Link>
          <Link to={`/products/${item.slug}`}><h4 className="related-item__title">{item.title}</h4></Link>
          <Link to={`/products/${item.slug}`}><h5 className="related-item__group">{item.group}</h5></Link>
